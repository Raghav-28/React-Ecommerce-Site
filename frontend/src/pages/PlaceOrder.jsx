...
const initPay = (order)=>{
  const options = {
    ...
    handler: async (response)=>{
      ...
      try {
        const response = await axios.post('http://localhost:4000/api/order/verifyRazorpay', response,{headers:{token}})
        if(data.success){
          navigate('/orders')
          setCartItems({})
        }
    ...
    case 'cod':
      ...
      case 'razorpay':
        ...
        if(responseRazorpay.data.success){
          ...
          alert(responseRazorpay.data.message);
        }
      break;
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-[480px]">
        ...
        <div  className="flex gap-3">
          ...
          <input onChange={onChangeHandler} name="phone" value={formData.phone} type="number" placeholder="PhoneNumber" className="border border-gray-300 rounded py-1.5 px-3.5 w-full" required/>
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            ...
            <div onClick={()=>setMethod('razorpay')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
              ...
              <img src={assets.razorpay_logo} className="h-5 mx-4 w-16 " alt="" />
            </div>
